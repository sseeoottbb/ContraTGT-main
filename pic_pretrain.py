...
early_stopping = EarlyStopping(dn=data_name, max_round=8, tolerance=1e-4)
alpha = 0.6
...
# === MODIFIED ===
# 创建所有必要的输出目录 (修复 RuntimeError)
os.makedirs('pretrain_model', exist_ok=True)
os.makedirs('middle_model', exist_ok=True)
os.makedirs('saved_checkpoints', exist_ok=True)  # EarlyStopping 会用到
os.makedirs('pretrain_pic', exist_ok=True)
# === MODIFIED END ===
...
# --- 新增：绘图函数 ---
def plot_and_save_pretrain_metrics():
    """绘制双轴折线图并保存数据和图片"""
    if not epochs_list:
    ...
    ax1.set_ylabel('Contrastive Loss', color=color)
    l1, = ax1.plot(epochs_list, train_losses_list, 'b-', label='Contrastive Loss', color=color)
    ax1.tick_params(axis='y', labelcolor=color)

    # 创建第二个 Y 轴用于 Consistency 和 Diversity (右轴)
    ax2 = ax1.twinx()
    color = 'tab:orange'
    ax2.set_ylabel('Augmentation Metrics', color=color)
    l2, = ax2.plot(epochs_list, con_losses_list, 'g--', label='Consistency Loss', color='green')
    l3, = ax2.plot(epochs_list, div_losses_list, 'r-.', label='Diversity Loss', color='red')
    ax2.tick_params(axis='y', labelcolor=color)

    # 设置标题和网格
    plt.title(f'Pre-training Dynamics - {data_name}')
    ...
    # 保存图片
    pic_path = f'pretrain_pic/pretrain_dynamics_{data_name}.png'
    plt.savefig(pic_path, dpi=300, bbox_inches='tight')
    print(f"Pre-training plot saved to {pic_path}")

    # 保存数据到 CSV
    data_df = pd.DataFrame({
        'Epoch': epochs_list,
        'Contrastive_Loss': train_losses_list,
        'Consistency_Loss': con_losses_list,
        'Diversity_Loss': div_losses_list
    })
    csv_path = f'pretrain_pic/pretrain_dynamics_data_{data_name}.csv'
    data_df.to_csv(csv_path, index=False)
    print(f"Pre-training data saved to {csv_path}")
...
for epoch in tqdm(range(n_epoch)):
        ...
        for se in range(6):
            ...
            with torch.no_grad():
                ...
                pos, neg = model.linkPredict(spa_src_feature, spa_src_mask, tmp_src_feature, tmp_src_mask,
                                             spa_dst_feature, spa_dst_mask, tmp_dst_feature, tmp_dst_mask, fake_ctx_fea,
                                             fake_ctx_mask, fake_tmp_fea, fake_tmp_mask)
            ...
            # === MODIFIED ===
            # 方案A (原代码): 基于预测概率的一致性 (可能导致不下降)
            # pos_prob, neg_prob = mid_model.linkPredict(...)
            # con_loss = criterion(pos_prob, pos_label) + criterion(neg_prob, neg_label)

            # 方案B (推荐): 基于嵌入表示的一致性 (更直接)
            h_orig_spa = mid_model.get_seq_embed(spa_fea_1, spa_mask_1, 'spatial')
            h_aug_spa = mid_model.get_seq_embed(spa_feature_1, spa_mask_1, 'spatial')
            h_orig_tmp = mid_model.get_seq_embed(tmp_fea_1, tmp_mask_1, 'temporal')
            h_aug_tmp = mid_model.get_seq_embed(tmp_feature_1, tmp_mask_1, 'temporal')
            con_loss_spa = torch.nn.functional.mse_loss(h_orig_spa, h_aug_spa.detach())
            con_loss_tmp = torch.nn.functional.mse_loss(h_orig_tmp, h_aug_tmp.detach())
            con_loss = con_loss_spa + con_loss_tmp
            # === MODIFIED END ===
    ...
    # --- 新增：在每个 epoch 结束后计算平均值并记录 ---
    avg_loss = np.average(train_loss)
    avg_top_loss = np.average(top_loss)
    avg_con_loss = np.average(divloss_1)
    avg_div_loss = np.average(divloss_2)

    # 记录到列表中
    epochs_list.append(epoch)
    train_losses_list.append(avg_loss)
    con_losses_list.append(avg_con_loss)
    div_losses_list.append(avg_div_loss)

    print('epoch ', epoch, 'train_loss:', avg_loss, ',top_loss:', avg_top_loss, 'con_loss:', avg_con_loss, 'div_loss:',
          avg_div_loss)
...
# --- 新增：训练结束后调用绘图函数 ---
plot_and_save_pretrain_metrics()
